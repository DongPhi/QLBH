...
class Product extends CI_Controller
{
    ...
    public function cms_load_listgroup()
    {
        ...
        $sls_group = $this->cms_nestedset->dropdown('products_group', NULL, 'manufacture');
        ob_start();
        echo '<option value="-1" selected="selected">--Lựa chọn--</option>';
            ...
            if ($option['option2'] == '-1') {
                if ($option['option3'] == '-1') {
                    ...
                    $data['data']['_list_product'] = $this->db
                        ->select('ID,prd_code,prd_name,prd_sls,prd_sell_price,prd_origin_price,prd_group_id,prd_manufacture_id,prd_image_url,prd_status')
                        ->from('products')
