...
class Orders extends CI_Controller
{
    ...
    public function cms_save_orders($store_id)
    {
        if($store_id==$this->auth['store_id']){
            ...
            $this->db->select_max('output_code')->like('output_code', 'HD');
            $max_output_code = $this->db->get('orders')->row();
            $max_code = (int)(str_replace('HD', '', $max_output_code->output_code)) + 1;
            if ($max_code < 10)
                $order['output_code'] = 'HD000000' . ($max_code);
            else if ($max_code < 100)
                $order['output_code'] = 'HD00000' . ($max_code);
            else if ($max_code < 1000)
                $order['output_code'] = 'HD0000' . ($max_code);
            else if ($max_code < 10000)
                $order['output_code'] = 'HD000' . ($max_code);
            else if ($max_code < 100000)
                $order['output_code'] = 'HD00' . ($max_code);
            else if ($max_code < 1000000)
                $order['output_code'] = 'HD0' . ($max_code);
            else if ($max_code < 10000000)
                $order['output_code'] = 'HD' . ($max_code);
